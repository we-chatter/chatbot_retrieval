...
class ProcessIntoES(object):
    def __init__(self, ip="127.0.0.1"):
        ...
        # self.music_file = os.path.join(cur, 'resource/qa_corpus.json')
        self.music_file = basedir + '/resource/qa_corpus.json'
    ...
    def insert_data_bulk(self, action_list):
        ...
        logging.info("Performed {0} actions. _: {1}".format(success, _))
...
def init_ES():
    ...
    for line in open(pie.music_file, 'r', encoding='utf-8'):
        ...
        if index > BULK_COUNT:
            ...
            index = 0
            count += 1
            logging.info("bulk {} writted finished!".format(count))
            action_list = []
    end_time = time.time()

    logging.info("Time cost:{0}".format(end_time - start_time))
